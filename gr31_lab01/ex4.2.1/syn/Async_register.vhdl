...
package CONV_PACK_async_register_NBIT16 is

-- define attributes
attribute ENUM_ENCODING : STRING;

end CONV_PACK_async_register_NBIT16;

library IEEE;

use IEEE.std_logic_1164.all;

use work.CONV_PACK_async_register_NBIT16.all;
...
entity async_register_NBIT16 is

   port( D : in std_logic_vector (15 downto 0);  CK, RESET : in std_logic;  Q :
         out std_logic_vector (15 downto 0));

end async_register_NBIT16;

architecture SYN_str of async_register_NBIT16 is
...
begin
   ...
   async_fd_0 : FD_0 port map( D => D(0), CK => CK, RESET => RESET, Q => Q(0));
   async_fd_1 : FD_15 port map( D => D(1), CK => CK, RESET => RESET, Q => Q(1))
                           ;
   async_fd_2 : FD_14 port map( D => D(2), CK => CK, RESET => RESET, Q => Q(2))
                           ;
   async_fd_3 : FD_13 port map( D => D(3), CK => CK, RESET => RESET, Q => Q(3))
                           ;
   async_fd_4 : FD_12 port map( D => D(4), CK => CK, RESET => RESET, Q => Q(4))
                           ;
   async_fd_5 : FD_11 port map( D => D(5), CK => CK, RESET => RESET, Q => Q(5))
                           ;
   async_fd_6 : FD_10 port map( D => D(6), CK => CK, RESET => RESET, Q => Q(6))
                           ;
   async_fd_7 : FD_9 port map( D => D(7), CK => CK, RESET => RESET, Q => Q(7));
   async_fd_8 : FD_8 port map( D => D(8), CK => CK, RESET => RESET, Q => Q(8));
   async_fd_9 : FD_7 port map( D => D(9), CK => CK, RESET => RESET, Q => Q(9));
   async_fd_10 : FD_6 port map( D => D(10), CK => CK, RESET => RESET, Q => 
                           Q(10));
   async_fd_11 : FD_5 port map( D => D(11), CK => CK, RESET => RESET, Q => 
                           Q(11));
   async_fd_12 : FD_4 port map( D => D(12), CK => CK, RESET => RESET, Q => 
                           Q(12));
   async_fd_13 : FD_3 port map( D => D(13), CK => CK, RESET => RESET, Q => 
                           Q(13));
   async_fd_14 : FD_2 port map( D => D(14), CK => CK, RESET => RESET, Q => 
                           Q(14));
   async_fd_15 : FD_1 port map( D => D(15), CK => CK, RESET => RESET, Q => 
                           Q(15));
