	...
	begin
		...
		begin	
			first_block : if in_col = 1 generate
				signal i_k_tmp : std_logic_vector(1 downto 0);
			begin
				i_k_tmp <= Big_G(in_col)(in_col) & Big_P(in_col)(in_col);
			...
			other_blocks : if in_col > 1 and (in_col mod 2 = 1) generate
				signal i_k_tmp : std_logic_vector(1 downto 0);
				signal k_1_j_tmp : std_logic_vector(1 downto 0);
			begin
				i_k_tmp <= Big_G(in_col)(in_col) & Big_P(in_col)(in_col);
				k_1_j_tmp <= Big_G(in_col-1)(in_col-1) & Big_P(in_col-1)(in_col-1);
		...
		begin			
			...
			begin
				...
				-- The position of the g_block has an exponential dependance on the row index (in_row).
				-- As a matter of
				-- fact the g_block is positioned at 2^in_row column:
					...
					first_block : if in_col = (2**in_row) - 1 generate
						signal i_k_tmp : std_logic_vector(1 downto 0);
					begin
						i_k_tmp <= Big_G(in_col)(in_col-2**(in_row-1)+1) & Big_P(in_col)(in_col-2**(in_row-1)+1);
						...
						g12 : entity work.g_block
							...
							-- In here we can also assign the output of the carry out vector.
							-- For rows 2 and 3, that have only 1
					...
					begin
						i_k_tmp <= Big_G(in_col)(in_col-2**(in_row-1)+1) & Big_P(in_col)(in_col-2**(in_row-1)+1);
						k_1_j_tmp <= Big_G(in_col-2**(in_row-1))(in_col-2**in_row+1) & Big_P(in_col-2**(in_row-1))(in_col-2**in_row+1);
					...
					begin
						...
						begin
							...
							first_parent_children : if in_col = (2**in_row) - 1 generate
								signal i_k_tmp : std_logic_vector(1 downto 0);
							begin
								i_k_tmp <= Big_G(k)(in_col+1) & Big_P(k)(in_col+1);
							...
							begin
								i_k_tmp <= Big_G(k)(in_col+1) & Big_P(k)(in_col+1);
								k_1_j_tmp <= Big_G(in_col)(in_col - 2**in_row + 1) & Big_P(in_col)(in_col - 2**in_row + 1);
